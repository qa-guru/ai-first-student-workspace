...
SSOT визуала: `@zero-design-system/react` → `vendor/frontend-react-ui`. CSS lean runtime: `vendor/ds/css/`, подключается из `src/styles.ts`.
...
Якоря testid (не ломать без смены PO): `login-form`, `submit-button`, `error-message`, `health-panel`, `items-list` — таблица в `frontend/typescript/frontend-typescript-react/README.md`. `note-panel` / `note-save-button` — только ветка `develop` (на `main` панели нет).
