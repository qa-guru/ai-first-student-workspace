...
**Открыть:** [index.html](index.html) — клик по строке прыгает к листу **на этой же странице**. Превью Cursor не переходит на другой локальный файл; якоря `#…` работают.
...
- [00 · Общая карта](index.html#00-overview) — четыре вопроса, чайник = автотест на логин, quiz
...
Промпт: «покрой неверный пароль». Слева — ответ без слоя, справа — канон.
