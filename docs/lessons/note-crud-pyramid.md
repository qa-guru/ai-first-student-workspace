...
| 8 | e2e | `NoteTests` + PO `note-panel` в `HomePage`; fluent login → панель → save (**PUT**); не PATCH/415/401/схема; без `@Tag("smoke")` | `cd tests/java/tests-java-gradle-junit5-allure3-selenide && ./gradlew test -Denv=ci -DincludeTags=e2e -DexcludeTags=screenshot -Dtest=NoteTests` | 0 | 1 тест: фабрика (не `user1`) → вижу панель → save PUT create; Delete enabled; ошибка скрыта |
