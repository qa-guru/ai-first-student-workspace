...
| screenshot ci refresh | `SCREENSHOT_BROWSER=chrome ./gradlew test -Denv=ci -DincludeTags=screenshot -DupdateScreenshots=true` | writes `screenshots/prod/<os>/chrome-148/` · CI job `ci-screenshot-refresh` (`update_ci_screenshots`) |
| screenshot prod refresh | `SCREENSHOT_BROWSER=chrome ./gradlew test -Denv=prod -DincludeTags=screenshot -DupdateScreenshots=true` | writes `screenshots/prod/linux/chrome-148/` · live prod stand, not pipeline compose |
